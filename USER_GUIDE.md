...
Public registration creates a **customer** account only (name, email, password — min 6 chars). No email verification; you're logged in immediately. There is **no Register button in the navbar** — the logged-out top bar shows only Login; customers reach the form via the "Register" link on the login page (or `/register` directly). **Business-owner accounts are created by an admin**, not self-registered — see the admin Users section below.

### Contact (`/contact`)
An info / "contact us" page in the top navbar with SmartBiz's phone, office, and hours, plus a message form. Submitting posts to `POST /api/contact`, which **emails the message to the SmartBiz inbox** (`CONTACT_TO`, default `nuby.chartine@gmail.com`). Email delivery uses whichever is configured: **Brevo HTTP API** (`BREVO_API_KEY` + `BREVO_SENDER`) or **SMTP** (`SMTP_HOST`/`SMTP_USER`/`SMTP_PASS` — Gmail app password). Use Brevo on hosts that block outbound SMTP (e.g. Render's free tier, which times out on port 465); SMTP is fine locally. Without either, messages are logged server-side and the form still confirms receipt. The same delivery is used for the business-owner **welcome email**.

### Top bar (logged in)
Once logged in, the Home / Businesses / Pricing menu disappears (it's for the landing pages only) and the top bar shows just a **round profile button with your initials**. Clicking it opens a card with your name, email, role badge, a **Dashboard** link for the roles that have one (Super Admin → `/superadmin`, Business Owner → `/dashboard`; customers see no dashboard link), an **Edit profile** button (update your name, email, or password), and a **Logout** button.
...
- **+ Add Business** opens an inline form (Name, Description, Category, City are required; City pre-fills "Kigali").
  - **Category** — pick from the list, or click **+ New category** to create one on the spot if the category you need isn't listed (duplicates are rejected). The new category is selected automatically and becomes available to everyone.
  - **Business picture (optional)** — upload your own cover photo, or leave it blank to use an automatic image based on the category. You can also change it later from the browse page (hover your business's cover).
...
- **Businesses** — searchable, paginated table of all ~90+ businesses (10/page). The super admin can **delete** any business (moderation, with confirmation) but **cannot edit or add** businesses — owners create and edit their own. The Verified column is a green/grey dot — **read-only in the UI** (changeable only in the DB).
...
- **Users** — every account with role badges (purple = admin, green = owner, grey = customer). **Click any row** for a modal showing that user's businesses, plans and ratings. Each row has an **Actions** column with **edit** (change name, email, role) and **delete** (with confirmation; cascades to that user's businesses, reviews and subscriptions) — the super admin row is protected from both. An **Add Business Owner** button creates a business-owner account (name, email, temporary password); the owner then signs in and registers their own businesses from their dashboard. This is the only way business-owner accounts are created. On creation, a **welcome email with their login details** is sent to the owner's address (requires SMTP configured — see §2; otherwise it's logged and the success screen says so).
...
4. **As the admin, create a Business Owner** (Users → Add Business Owner), then log in as them → register their own business from the dashboard → find it on `/businesses` → edit it → **subscribe it to Premium** (choose MoMo or VISA at checkout — payment is simulated) → recheck `/businesses` and Home: it now has the green ring, "SPONSORED" pill, and top placement. Try **Edit profile** from the avatar menu too.
